...
MANIFEST = ROOT / "results/placebo_experiment_frozen.json"
...
for tid in args.targets:
    if tid not in targets:
        raise SystemExit(
            f"Unknown/non-frozen target: {tid}"
        )
...
for tid in args.targets:
    ...
    for pair in t["pairs"]:
        ...
        # Counterbalance generation order only from frozen seed.
        if left_seed % 2 == 0:
            arms = ["left", "right"]
        else:
            arms = ["right", "left"]
...
for job_idx, job in enumerate(
    ...
    # Same RNG seed for both members of placebo pair.
    random.seed(
        continuation_seed
    )
